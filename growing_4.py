...
class plant_model:
    ...
    def newton_step(self, x1, x2, t, dx1, dx2):
        dt = self.param['dt']
        f0 = self.scalar_calc_functional(x1, x2, t)
        f1 = self.scalar_calc_functional(x1 + dx1, x2, t+dt)
        f11 = self.scalar_calc_functional(x1-dx1, x2, t+2*dt)
        f2 = self.scalar_calc_functional(x1, x2 + dx2, t+3*dt)
        f22 = self.scalar_calc_functional(x1, x2 - dx2, t+4*dt)
        ...
        # find optimal gamma
        g = np.arange(0.0001, 1, 0.005)
        x1vec = np.ones(np.shape(g)[0])*x1
        x2vec = np.ones(np.shape(g)[0])*x2
        h1vec = np.ones(np.shape(g)[0])*((1/np.float(ddf1*ddf2))*(ddf2*df1+0*df2))
        h2vec = np.ones(np.shape(g)[0])*((1/np.float(ddf1*ddf2))*(ddf1*df2+0*df1))
        func = self.vector_calc_fuctional(x1vec+g*h1vec, x2vec+g*h2vec, t+4*dt)
        gamma = g[np.argmin(func)]
        # optimal result
        x1_next = x1+(gamma/np.float(ddf1*ddf2))*(ddf2*df1+0*df2)
        x2_next = x2+(gamma/np.float(ddf1*ddf2))*(ddf1*df2+0*df1)

        return x1_next, x2_next

    def find_newton_minimum(self, max_iteration_number = 100, x_start = 20, y_start = 20):
        '''
        ...
        for i in range(0, max_iteration_number):
            ...
            y_min = np.append(y_min, Y_[n_min])
            # newton step
            xg, yg = self.newton_step(x_grad[i], y_grad[i], t, dx1, dx2)
            x_grad = np.append(x_grad, xg)
            y_grad = np.append(y_grad, yg)
            if(i%25==0):
                # plot all steps
                ...
                pl.plot(X_[n_min], Y_[n_min], "sk")
                axes = pl.gca()
                draw_triangle(axes, (0, 0), (0, 1), (1, 0))
                pl.grid()
                pl.show()
            print(x_grad[i], y_grad[i], t)
            print(x_min[i],  y_min[i])
...
if __name__ == "__main__":
    ...
    ##x_start = 5
    ##y_start = 5
    ### main cycle for metaoptimization
    ##mx = [[0.1, 0.1, 0.1]]
    ##mgamma = 0.5
    ##dmx1 = 0.5
    ##dmx2 = 0.5
    ##dmx3 = 0.5
    ##params = {'metagamma':mgamma, 'start_point':(x_start, y_start),\
        ##'deltas':[dmx1, dmx2, dmx3], 'start_metas':mx[0]}
    ##global_data = {'parameters':params}
    ##for i in range(0, 50):
        ##print("iteration number {}".format(i))
        ##error0 = plant.find_triangle_minimum(x_start = 5, y_start = 5, show = False,\
        ##alpha = mx[i][0], beta = mx[i][1], gamma = mx[i][2])
        ##error1 = plant.find_triangle_minimum(x_start = 5, y_start = 5, show = False,\
        ##alpha = mx[i][0]+dmx1, beta = mx[i][1], gamma = mx[i][2])
        ##error2 = plant.find_triangle_minimum(x_start = 5, y_start = 5, show = False,\
        ##alpha = mx[i][0], beta = mx[i][1]+dmx2, gamma = mx[i][2])
        ##error3 = plant.find_triangle_minimum(x_start = 5, y_start = 5, show = False,\
        ##alpha = mx[i][0], beta = mx[i][1], gamma = mx[i][2]+dmx3)
        ##epoch = {'iteration':i, 'metas':mx[i], 'results':[error0, error1, error2, error3]}
        ##key = 'epoch_{}'.format(i)
        ##global_data[key] = epoch
        ###fig = pl.figure()
        ###pl.plot(range(0, np.shape(error0)[0], 1), error0[:], "-or")
        ###pl.grid()
        ###pl.show()
        ###pl.plot(range(0, np.shape(error1)[0], 1), error1[:], "-ob")
        ###pl.grid()
        ###pl.show()
        ###pl.plot(range(0, np.shape(error2)[0], 1), error2[:], "-og")
        ###pl.grid()
        ###pl.show()
        ###pl.plot(range(0, np.shape(error3)[0], 1), error3[:], "-oc")
        ###pl.grid()
        ###pl.show()
        ###print(error0)
        ###print(error1)
        ...
        ##E2 = np.mean(error2)
        ##E3 = np.mean(error3)
        ##print("Iteration {}, E0 = {}, E1 = {}, E2 = {}, E3 = {}, a = {}, b = {}, g = {} \n".format\
            ##(i, E0, E1, E2, E3, mx[i][0], mx[i][1], mx[i][2]))
        ##mx1_next = mx[i][0]-mgamma*((E1 - E0)/np.float(dmx1))
        ##mx2_next = mx[i][1]-mgamma*((E2 - E0)/np.float(dmx2))
        ##mx3_next = mx[i][2]-mgamma*((E3 - E0)/np.float(dmx3))
        ##print(mx1_next, mx2_next, mx3_next)
        ##mx.append([mx1_next, mx2_next, mx3_next])
        ### save results
        ##with open('results2.txt', 'a') as f:
            ##print("Iteration {}, E0 = {}, E1 = {}, E2 = {}, E3 = {}, a = {}, b = {}, g = {} \n".format\
            ##(i, E0, E1, E2, E3, mx[i][0], mx[i][1], mx[i][2]), file = f)
    ##with open('triangle_metaopt_all_data.pickle', 'wb') as f:
