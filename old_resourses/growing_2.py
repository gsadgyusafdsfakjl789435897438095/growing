...
def newton(x1, x2, t, dx1, dx2):
    dt = 1 #dt = 20 minuts and it is one
    f0 = fp(x1, x2, t)
    f1 = fp(x1 + dx1, x2, t+dt)
    f11 = fp(x1-dx1, x2, t+2*dt)
    f2 = fp(x1, x2 + dx2, t+3*dt)
    f22 = fp(x1, x2 + dx2, t+4*dt)
    ...
    x1_next = x1-(1/np.float(ddf1*ddf2))*(ddf2*df1+0*df2)#must be corrected
    x2_next = x2-(1/np.float(ddf1*ddf2))*(ddf1*df2+0*df1)#must be corrected
